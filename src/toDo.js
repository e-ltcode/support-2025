const Zovi = () => {
   ...
   const DBOpenRequest = window.indexedDB.open('ToDoDB', 4);
   ...
   DBOpenRequest.onupgradeneeded = (event) => {
      ...
      const objectStore = db.createObjectStore('QuestionGroup', { keyPath: 'taskTitle' });
   ...
   function addQuestions() {
      ...
      questions.forEach(q => {
         const objectStoreRequest = objectStore.add(q);
         objectStoreRequest.onsuccess = (event) => {
            console.log('dodao question', q)
         }
      })
   }
   ...
   function addData(e) {
      ...
      const objectStore = transaction.objectStore('QuestionGroup');
   ...
   function deleteItem(event) {
      ...
      const transaction = db.transaction(['QuestionGroup'], 'readwrite');
      transaction.objectStore('QuestionGroup').delete(dataTask);
   ...
   function checkDeadlines() {
      ...
      const objectStore = db.transaction(['QuestionGroup'], 'readwrite').objectStore('QuestionGroup');
